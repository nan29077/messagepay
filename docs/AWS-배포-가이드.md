...
DIRECT_DATABASE_URL="postgresql://...rds.amazonaws.com:5432/tornado?sslmode=require" \
...
```bash
SHADOW_DATABASE_URL="postgresql://user:pw@host:5432/tornado_shadow" \
...
CREATE ROLE tornado_migrate LOGIN PASSWORD '...';
GRANT ALL ON SCHEMA public TO tornado_migrate;
ALTER SCHEMA public OWNER TO tornado_migrate;

-- 2) 앱 런타임 롤 (DML 만). 애플리케이션이 사용
CREATE ROLE tornado_app LOGIN PASSWORD '...';
GRANT USAGE ON SCHEMA public TO tornado_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO tornado_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO tornado_app;
ALTER DEFAULT PRIVILEGES FOR ROLE tornado_migrate IN SCHEMA public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO tornado_app;

-- 3) 원장은 앱 롤에서 INSERT/SELECT 만 허용 (트리거와 이중 방어)
REVOKE UPDATE, DELETE ON settlement_ledger FROM tornado_app;
```

- `DATABASE_URL` → `tornado_app`
- `DIRECT_DATABASE_URL` → `tornado_migrate`
...
```bash
STORAGE_DRIVER=s3
S3_BUCKET=donaido-media-prod
S3_PREFIX=uploads
S3_PUBLIC_BASE=https://media.donaido.kr   # CloudFront 도메인 (선택, 없으면 /api/media 경유)
