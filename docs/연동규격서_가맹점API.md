...
> **이 연동은 선택 사항입니다.**
> 가맹점은 메시지페이 관리자 화면(문자 관리)에서 충전 내역을 확인하고 포인트 지급 처리를
...
| 구분 | 담당 |
| --- | --- |
| 문자 수신(MO)·문자 발송(MT) | 메시지페이 |
| 결제 승인·취소 | 메시지페이 (PG 연동) |
| 정산·지급 | 메시지페이 (지급대행) |
| 상품 등록·가격·재고·배송비 설정 | 가맹점 (메시지페이 관리자에서 설정) |
| **포인트·상품권·이용권 발행과 지급** | **가맹점** |
| **실물 상품 출고·배송** | **가맹점** |
| 회원 식별 | 가맹점 (휴대폰 번호 매칭) |

포인트는 **가맹점이 발행하는 자산**입니다. 메시지페이는 이용자의 결제를 대행하고 그 대금을
가맹점에 정산할 뿐, 포인트를 보관하거나 발행하지 않습니다.

### 1.2 연동 방식

가맹점 서버가 메시지페이를 **주기적으로 호출(pull)** 합니다.
메시지페이가 가맹점 서버를 호출하지 않으므로, 가맹점 쪽에 외부 수신 서버(webhook endpoint)를
따로 열지 않아도 됩니다.
...
메시지페이 가맹점 관리자 → **결제 설정 → 연동 API** 에서 발급합니다.
...
두 값 모두 메시지페이 서버에 **원문으로 저장되지 않습니다**(키는 해시, 서명 비밀키는 암호화).
...
X-Messagepay-Timestamp: 1790000000        (POST 필수, GET 선택)
X-Messagepay-Signature: <hex>             (POST 필수, GET 선택)
```
...
| 필드 | 설명 |
| --- | --- |
| `transactionNo` | 메시지페이 거래번호. **ack 에 이 값을 씁니다.** |
| `amount` / `points` | 결제 금액(원)과 지급할 포인트. **1:1** 입니다. |
| `payerPhone` | 이용자 휴대폰 번호. 회원 매칭 기준 |
| `payerRef` | 번호를 저장하지 않으려는 가맹점용 고정 식별자. **가맹점마다 다른 값**이며, 같은 가맹점 안에서는 같은 번호 = 같은 값 |
...
- 한 번 `SENT` 로 확정된 건은 API 로 되돌릴 수 없습니다. 착오 적립은 메시지페이
  관리자에게 문의하세요.
...
4. `payerPhone`(또는 `payerRef`)로 회원을 찾는다.
   - 회원을 찾지 못하면 적립을 보류하고 `FAILED` + 사유로 통보한다.
     (메시지페이 관리자 화면에도 보류 사유가 표시되어 CS 응대가 쉬워집니다.)
...
- 환불은 메시지페이 관리자에서 처리하며, 환불된 건은 정산 원장에서 차감됩니다.
...
- 환불 사실은 메시지페이 관리자 화면의 결제 상세와 `status=all` 조회의
...
- 번호를 보관하고 싶지 않다면 `payerRef`(고정 식별자)만 저장해 회원과 매핑하세요.
  `payerRef` 는 가맹점마다 다른 값이 발급되므로, 다른 가맹점의 `payerRef` 와 대조해도
  같은 사람인지 알 수 없습니다(가맹점 간 이용자 추적 방지).
