...
export default async function HowItWorksPage() {
  ...
  return (
    <PublicShell aside={<HowAside />}>
      <PageHeader
        eyebrow="이용방법"
        title="문자 한 통으로 결제하는 전체 과정"
        description="최초 1회 계좌 등록만 마치면, 그 다음부터는 문자를 보내는 것만으로 결제가 접수됩니다. 아래 순서를 그대로 따라 주세요."
      />

      <Notice tone="warning" title="시작하기 전에 꼭 확인해 주세요">
        <ul className="list-disc space-y-1 pl-4">
          <li>최초로 보내는 문자는 결제 처리되지 않습니다. 계좌 등록 안내만 발송됩니다.</li>
          <li>등록을 마친 뒤 문자를 보내면 등록한 계좌에서 결제 금액이 출금됩니다.</li>
          <li>결제되지 않은 요청은 가맹 서비스에 충전으로 반영되지 않습니다.</li>
        </ul>
      </Notice>

      {/* 단계 안내 */}
      <section className="mt-8">
        <SectionTitle title="결제 절차" description="처음 한 번만 등록하면 이후에는 2단계부터 건너뜁니다." />
        <div className="space-y-2.5">
          <Step
            no={1}
            icon={<MessageSquare size={18} strokeWidth={1.7} />}
            title="가맹점 결제 수신번호로 문자 보내기"
            body="가맹 서비스 화면이나 결제 페이지에 안내된 번호로 문자를 보냅니다. 대표번호를 쓰는 가맹점은 메시지 맨 앞에 안내된 키워드를 붙여야 결제 대상이 구분됩니다."
            note="이 최초 문자는 결제로 접수되지 않으며, 결제도 발생하지 않습니다."
            noteTone="warning"
          />
          <Step
            no={2}
          ...
          <Step
            ...
            body="등록을 마친 뒤 다시 문자를 보내면 결제 PIN 입력 링크가 도착합니다. 결제사 화면에서 PIN 을 입력하는 순간 등록한 계좌에서 결제 금액이 출금됩니다."
          ...
          <Step
            ...
            title="가맹 서비스에 충전 반영"
            body="결제가 완료된 건만 가맹 서비스에 충전으로 반영되고 완료 문자를 받습니다. 처리 결과는 마이페이지에서 확인할 수 있습니다."
        ...
        <Card>
          <div className="space-y-3">
            <Line
              ...
              tone="warning"
              title="최초 문자 — 출금 없음"
              body="계좌 등록 전에 보낸 문자는 결제로 처리되지 않으며 요금이 청구되지 않습니다. (이동통신사 문자 발송 요금은 별도)"
            />
            <Line
              icon={<Landmark size={17} strokeWidth={1.7} />}
              tone="danger"
              title="등록 후 문자 — PIN 입력 시 출금"
              body="계좌 등록을 마친 뒤 보내는 문자는 결제 PIN 인증을 거쳐 등록한 계좌에서 결제 금액이 출금됩니다."
            />
            <Line
              icon={<Clock size={17} strokeWidth={1.7} />}
              tone="neutral"
              title="PIN 을 입력하지 않은 요청 — 자동 만료"
              body="결제 PIN 을 입력하지 않으면 요청은 만료되고 출금되지 않습니다. 만료된 요청은 충전으로도 반영되지 않습니다."
            />
          </div>
        </Card>
      </section>
      ...
      <section className="mt-8">
        <SectionTitle
          title="이용 한도"
          description="과도한 결제를 막기 위한 기본 한도입니다. 마이페이지에서 더 낮게 설정할 수 있습니다."
        />
        <Card>
        ...
        <p className="mt-2 flex gap-2 text-[12px] leading-relaxed text-ink-400">
          ...
          <span>
            한도를 넘는 문자는 결제로 접수되지 않고 안내 문자가 발송됩니다. 이미 지난 결제는 한도 변경의 영향을 받지
            않습니다.
          </span>
        </p>
      </section>
        ...
        <Card>
          <div className="space-y-3">
            <Line
              ...
              tone="neutral"
              title="환불 요청 방법"
              body="마이페이지 결제 내역에서 결제 완료 건의 환불을 요청하거나, 고객센터로 거래번호와 함께 접수해 주세요."
            />
            <Line
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[13px] leading-relaxed text-ink-500">
                결제 금액과 별개로 이동통신사의 문자 발송 요금이 발생할 수 있습니다. 요금 정책은 가입한 통신 요금제를
                확인해 주세요.
              </p>
            </div>
          </div>
      ...
      <section className="mt-8">
        <Card className="bg-ink-900 text-white">
          <p className="text-[15px] font-extrabold leading-snug">결제할 가맹점의 코드를 알고 계신가요?</p>
          <p className="mt-1.5 text-[13px] leading-relaxed opacity-90">
            가맹 서비스 화면에 안내된 코드를 입력하면 결제 수신번호와 안내를 바로 확인할 수 있습니다.
          </p>
    ...
    <div className="sticky top-24 space-y-3">
      <Card>
        ...
        <ol className="mt-3 space-y-2 text-[13px] leading-relaxed text-ink-500">
          <li>1. 결제 수신번호로 문자 발송</li>
          <li>2. 안내 링크에서 계좌 등록</li>
          <li>3. 문자 재발송 후 결제 확인</li>
          <li>4. 가맹 서비스에 충전 반영</li>
        </ol>
      </Card>
      <Card>
