...
import {
  ...
  ShoppingBag, Smartphone, Sparkles, WalletCards, Zap,
} from 'lucide-react';
...
export default function HomePage() {
  return (
    <PublicShell>
      <section className="munjapay-hero relative isolate overflow-hidden rounded-[30px] bg-[#071426] text-white shadow-[0_28px_80px_rgba(7,20,38,0.28)]">
        <Image src="/assets/munjapay-hero-v1.png" alt="문자 메시지를 통해 포인트 결제가 완료되는 문자페이 서비스" fill priority sizes="(min-width: 768px) 640px, 100vw" className="object-cover object-[64%_center] opacity-80" />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(7,20,38,1)_0%,rgba(7,20,38,.94)_40%,rgba(7,20,38,.2)_100%)]" />
        <div className="relative flex min-h-[520px] flex-col justify-end px-6 py-8 sm:min-h-[570px] sm:px-9 sm:py-10">
          <span className="mb-auto inline-flex w-fit items-center gap-2 rounded-full border border-[#b7f34a]/30 bg-[#b7f34a]/10 px-3 py-1.5 text-[11px] font-extrabold tracking-[0.12em] text-[#d8ff88]">
            <Sparkles size={14} strokeWidth={1.7} /> SMS PAYMENT, SIMPLIFIED
          </span>
          <div className="max-w-[430px]">
            <p className="text-[13px] font-bold text-[#b7f34a]">쉽고 빠른 문자결제</p>
            <h1 className="mt-3 text-[38px] font-black leading-[1.08] tracking-[-0.06em] sm:text-[48px]">문자 한 통으로,<br />충전이 끝납니다</h1>
            <p className="mt-4 max-w-[390px] text-[14px] leading-[1.75] text-white/68 sm:text-[15px]">별도 앱을 찾고 복잡한 메뉴를 거칠 필요 없이, 문자를 보내고 확인하면 필요한 포인트가 바로 충전됩니다.</p>
            <div className="mt-6 flex flex-wrap gap-2.5">
              <Link href="/how-it-works" className="inline-flex h-12 items-center gap-2 rounded-full bg-[#b7f34a] px-5 text-[14px] font-extrabold text-[#071426] transition-transform hover:-translate-y-0.5">이용방법 보기<ArrowRight size={16} strokeWidth={1.9} /></Link>
              <Link href="/support" className="inline-flex h-12 items-center gap-2 rounded-full border border-white/20 bg-white/8 px-5 text-[14px] font-bold text-white backdrop-blur transition-colors hover:bg-white/14">서비스 도입 문의</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="for-business" className="mt-12 scroll-mt-24">
      ...
      <section className="mt-12 overflow-hidden rounded-[28px] bg-[#071426] px-5 py-6 text-white sm:px-7 sm:py-8">
      ...
      <section className="mt-12 overflow-hidden rounded-[28px] bg-[#b7f34a] p-6 text-[#071426] sm:p-8">
        ...
        <p className="mt-3 max-w-[430px] text-[13px] leading-relaxed text-[#071426]/68">문자페이를 내 서비스에 연결하고 싶다면 필요한 충전 방식과 운영 환경을 알려주세요.</p>
        <LinkButton href="/support" variant="secondary" size="lg" className="mt-5 !border-[#071426]/10 !bg-[#071426] !text-white hover:!bg-[#102846]">도입 상담 시작하기<ArrowRight size={16} strokeWidth={1.8} /></LinkButton>
      </section>
