 ...
 * 결제 내역에 표시될 이름 수정 폼.
 *
 * 이용자는 휴대폰 번호만 남기므로, 닉네임을 정하지 않으면 가맹점 화면과
 * 결제 내역에 "이용자5678" 처럼 표시된다. 그래서 결과를 항상 미리 보여준다.
 */
      ...
      <form action={action} className="space-y-3">
        ...
        <div className="rounded-xl bg-brand-50 px-3.5 py-3">
          ...
          <p className="mt-1 text-[14px] font-bold text-ink-900">
            {preview}님이 3,000원을 결제하셨습니다
          </p>
          {usingDefault ? (
            <p className="mt-1.5 text-[11.5px] leading-relaxed text-ink-500">
              지금은 휴대폰 번호 끝 4자리로 표시되고 있습니다. 닉네임을 정하면 가맹점이 누가
              보냈는지 알아볼 수 있습니다.
            </p>
          ) : null}
        </div>
        ...
        <p className="text-[11.5px] leading-relaxed text-ink-400">
          이미 접수된 결제는 그때 표시된 이름이 그대로 남습니다. 바꾼 닉네임은 다음 결제부터 적용됩니다.
        </p>
      </form>
    </Card>
  );
