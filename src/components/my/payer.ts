 ...
 * - PayerProfile 은 문자결제(MO) 수신 시 생성되므로 회원가입만 한 계정에는 없을 수 있다.
 */
...
export async function requirePayerContext(next = '/my'): Promise<PayerContext> {
...
export const NO_PAYER_DESC =
  '문자결제 내역은 휴대전화 번호를 기준으로 기록됩니다. 마이페이지의 내 정보 탭에서 본인 휴대폰 번호를 인증하면 그 번호로 결제한 내역이 이 계정에 표시됩니다.';
