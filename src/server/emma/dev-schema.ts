 ...
 * "수신 → 결제 → 충전 반영" 흐름을 그대로 돌려 보기 위한 것이다.
 *
...
export async function insertDevMo(
  input: DevMoInsert,
): Promise<{ moKey: string; moRecipient: string; emoRecipient: string | null }> {
  assertLocalOnly();
 ...
 * 문자를 실제로 넣지 않고 계산만 하므로 결제가 생기지 않는다.
 *
