...
export function tplRegisterGuide(creatorName: string, link: string): TemplateOutput {
  const text = withLink(
    `[토네이도] ${creatorName} 크리에이터 문자후원을 이용하려면 계좌 등록과 이용 동의가 필요합니다. 최초 문자는 후원 처리되지 않았습니다. 등록:`,
    link,
  );
...
export function tplConfirmPayment(creatorName: string, amount: bigint, link: string, ttlMin: number): TemplateOutput {
  const text = withLink(
    `[토네이도] ${creatorName} 크리에이터에게 ${formatNumber(amount)}원을 후원하시려면 아래 링크에서 확인해 주세요. ${ttlMin}분 내 미확인 시 자동 취소됩니다. 확인:`,
    link,
  );
...
export function tplDonationSuccess(input: {
  ...
  const text =
    `[토네이도] ${input.creatorName} 크리에이터에게 ${formatNumber(input.amount)}원이 후원되었습니다. ` +
...
export function tplDonationFailed(creatorName: string, reason?: string): TemplateOutput {
  const text =
    `[토네이도] ${creatorName} 크리에이터 후원이 완료되지 않았습니다. ` +
...
export function tplLimitBlocked(creatorName: string, reason: string): TemplateOutput {
  const text = `[토네이도] ${creatorName} 크리에이터 후원이 제한되었습니다. 사유: ${reason} 결제는 진행되지 않았습니다.`;
...
export function tplContentBlocked(creatorName: string): TemplateOutput {
  const text = `[토네이도] ${creatorName} 크리에이터에게 보낸 메시지가 운영정책에 따라 차단되었습니다. 결제는 진행되지 않았습니다.`;
...
export function tplRefundDone(creatorName: string, amount: bigint): TemplateOutput {
  const text = `[토네이도] ${creatorName} 크리에이터 후원 ${formatNumber(amount)}원이 취소되어 환불 처리되었습니다.`;
  return { code: MT_TEMPLATE.REFUND_DONE, text, masked: text };
}

export function tplUnknownRoute(): TemplateOutput {
  const text =
    '[토네이도] 후원 대상 크리에이터를 찾을 수 없습니다. 방송 화면에 안내된 번호와 코드를 다시 확인해 주세요. 결제는 진행되지 않았습니다.';
